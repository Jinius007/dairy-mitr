...
// Used to generate precise ration advisories when structured animal data is provided.
...
export function formatRationAdvisory(
  breedName: string,
  milkKg: number,
  fat: number,
  region: Region,
  result: ReturnType<typeof buildRation> & { req: ReturnType<typeof calcRequirements> },
  count = 1,
): string {
  const lines = result.ration.map((r) =>
    `- ${r.name}: ${r.asFeKg} kg/animal/day${count > 1 ? ` (${(r.asFeKg * count).toFixed(1)} kg for ${count} animals)` : ""} — ₹${r.cost.toFixed(2)}/animal`
  );
  return [
    `Ration advisory for ${breedName}${count > 1 ? ` × ${count} animals` : ""}`,
    `Milk: ${milkKg} kg/day, Fat: ${fat}%, 4% FCM: ${result.req.fcm.toFixed(1)} kg`,
    `Nutrient need — TDN: ${result.req.total.tdn.toFixed(0)} g, CP: ${result.req.total.cp.toFixed(0)} g, DM budget: ${(result.req.total.dm / 1000).toFixed(1)} kg`,
